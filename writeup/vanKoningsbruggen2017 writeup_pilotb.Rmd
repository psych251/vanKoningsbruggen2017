...
df.pilota = read.csv('PSYCH251 Pilot A_WithAMPText.csv', header = TRUE)

#### Data exclusion / filtering

df.data_exc <- df.pilota %>% 
  # filter(Q13 == "No") %>% #exclude participants familiar with chinese/japanese lang (NOT DOING THIS FOR PILOT A because one of two participants spoke chinese)
  filter(Q12 == "Yes")   #exclude participants who don't have FB
...
df.data_clean <- df.data_exc %>% 
  rename(chinese_lang = Q12,
         ...
         age = Q22,
         gender = Q23) %>% 
  mutate(fb_use_dur = recode(fb_use_dur, 
                            ...
                            `2-3 hours` = 5,
                            `3+ hours` = 6)) %>% 
  mutate(fb_use1 = recode(fb_use1, 
                            `Less than once a year` = 1,
                            `A few times a year` = 2,
                            `Less than once monthly` = 3,
                            `Once monthly` = 4,
                            `Twice monthly` = 5,
                            `Once weekly` = 6,
                            `2 or more times weekly` = 7,
                            `Once daily` = 8,
                            `Twice daily` = 9,
                            `3-4 times daily` = 10,
                            `More than 4 times daily` = 11)) %>% 
  mutate(fb_use2 = recode(fb_use2, 
                            `Less than once a year` = 1,
                            `A few times a year` = 2,
                            `Less than once monthly` = 3,
                            `Once monthly` = 4,
  ...
  mutate(fb_use6 = recode(fb_use6, 
                            ...
                            `3-4 times daily` = 10,
                            `More than 4 times daily` = 11)) %>% 
   mutate(craving1= recode(craving1, 
                            `Strongly disagree` = 1,
                            `Disagree` = 2,
                            `Neutral` = 3,
                            `Agree` = 4,
                            `Strongly agree` = 5)) %>% 
  mutate(craving2= recode(craving2, 
                            `Strongly disagree` = 1,
                            `Disagree` = 2,
                            `Neutral` = 3,
                            `Agree` = 4,
                            `Strongly agree` = 5)) %>% 
  mutate(craving3= recode(craving3, 
                            `Strongly disagree` = 1,
                            `Disagree` = 2,
                            `Neutral` = 3,
                            `Agree` = 4,
                            `Strongly agree` = 5)) %>% 
  mutate(craving4= recode(craving4, 
                            `Strongly disagree` = 1,
                            `Disagree` = 2,
                            `Neutral` = 3,
                            `Agree` = 4,
                            `Strongly agree` = 5)) %>% 
  mutate(craving5= recode(craving5, 
                            `Strongly disagree` = 1,
                            `Disagree` = 2,
                            `Neutral` = 3,
                            `Agree` = 4,
                            `Strongly agree` = 5)) %>% 
  mutate(craving6= recode(craving6, 
                            `Strongly disagree` = 1,
                            `Disagree` = 2,
                            `Neutral` = 3,
                            `Agree` = 4,
  ...
  mutate(fb_use_mean = mean(fb_use1, fb_use2, fb_use3, fb_use4, fb_use5, fb_use6)) %>%  #compute mean of all gen media use 
   mutate(cravings_mean = mean(craving1, craving2, craving3, craving4, craving5, craving6)) %>% 
  mutate(propFB = amp_sm_pleas / 30) %>% 
  mutate(propCon = amp_c_pleas / 30) %>%  

  #standardize fb_duration
  mutate(zscore = (fb_use_dur - mean(fb_use_dur))/sd(fb_use_dur)) %>% 
...
> Pilot B was a sample of 4 participants recruited from mTurk. 
...
#read in data
setwd("/Users/Angela/Desktop/vanKoningsbruggen2017")
df.pilotb = read.csv('PSYCH251 - SM AMP_PilotB_IdentDataRemoved.csv', header = TRUE)

#### Data exclusion / filtering

df.datab_exc <- df.pilotb %>% 
   filter(Q13 == "No") %>% #exclude participants familiar with chinese/japanese lang (NOT DOING THIS FOR PILOT A because one of two participants spoke chinese)
  filter(Q12 == "Yes")   #exclude participants who don't have FB
...
df.datab_clean <- df.datab_exc %>% 
  ...
  #standardize fb_use
  df.datab_clean$ztest_fb_use1 <- ((df.datab_clean$fb_use1 - mean(df.datab_clean$fb_use1)) / sd(df.datab_clean$fb_use1))
  df.datab_clean$ztest_fb_use2 <- ((df.datab_clean$fb_use2 - mean(df.datab_clean$fb_use2)) / sd(df.datab_clean$fb_use2))
   df.datab_clean$ztest_fb_use3 <- ((df.datab_clean$fb_use3 - mean(df.datab_clean$fb_use3)) / sd(df.datab_clean$fb_use3))
   df.datab_clean$ztest_fb_use4 <- ((df.datab_clean$fb_use4 - mean(df.datab_clean$fb_use4)) / sd(df.datab_clean$fb_use4))
   df.datab_clean$ztest_fb_use5 <- ((df.datab_clean$fb_use5 - mean(df.datab_clean$fb_use5)) / sd(df.datab_clean$fb_use2))
   df.datab_clean$ztest_fb_use6 <- ((df.datab_clean$fb_use6 - mean(df.datab_clean$fb_use6)) / sd(df.datab_clean$fb_use6))

   #mean of standardized fb 
   df.datab_clean <- df.datab_clean %>% 
...
#plot
#using original data NOT the model predictions they used in their fig 2

df.datab_clean$ID <- seq.int(nrow(df.datab_clean)) # add ID column
df.datab_tidy <- df.datab_clean %>% #make tidy 
  select(ID, propCon, propFB, zfb_use_mean, gender) %>% 
  pivot_longer(cols = -c(ID, zfb_use_mean, gender),
               names_to = "cue_type",
               values_to = "cue_prop")
library(ggplot2)

plot2 <- ggplot(data = df.datab_tidy,
...
library(tidyverse)
df.datab_corr <- df.datab_clean %>% 
  select(zfb_use_mean, propFB, propCon, cravings_mean) 

corrtable <- cor(df.datab_corr)
round(corrtable, 2)
...
plot1 <- ggplot(data = df.datab_clean,
                mapping = aes(x = zfb_use_mean, 
                              y = propFB)) + 
        geom_point() + 
        coord_cartesian(xlim = c(-2, 2), ylim = c(0,1))
...
model1 <- lmer(data = df.datab_tidy,
...
model2 <- lmer(data = df.datab_tidy,
               cue_prop ~ cue_type*zfb_use_mean*gender + (1| ID))

anova(model1, model2)

# Found a significant gender effect 
...
#testInteractions(model2.anova, 
#                 fixed= "df.datab_tidy$cue_type", 
#                 data = df.datab_tidy)
...
#total effect
fit.totaleffect = lm(cravings_mean ~ zfb_use_mean, df.datab_clean) 
summary(fit.totaleffect)

#IV on mediator 
fit.mediator = lm(amp_diff ~ zfb_use_mean, df.datab_clean) 
summary(fit.mediator)

#mediator on DV 
fit.dv = lm(cravings_mean ~ zfb_use_mean + amp_diff, df.datab_clean) 
summary(fit.dv)
...
# Could not test mediation because of insufficient power; gave an "essentially perfect fit" warning.  
```
