...
df.data_corr <- df.data_clean %>% 
  select(fb_use_mean, propFB, propCon, cravings_mean) 
...
library(lme4) 
df.data_clean %>%
  mutate(ResponseId = as.numeric(ResponseId))

model1 <- glmer(propFB ~ fb_use_mean + (1|ResponseId), df.data_clean, family = binomial)
...
model1b <- glmer(propFB ~ FBuse*meanuse*gender + (1|cuetype + subject), df.data_clean, family = binomial)
model2b <- glmer(propCon ~ FBuse*meanuse*gender + (1|cuetype + subject), df.data_clean, family = binomial)
